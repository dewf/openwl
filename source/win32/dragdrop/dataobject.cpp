...
ULONG __stdcall CDataObject::Release(void)
{
	...
	printf("releasing CDataObject [count %d]\n", count);
	if(count == 0)
	{
		delete this;
		return 0;
...
{
	if (renderFormat(pFormatEtc, pMedium)) {
		return S_OK;
	}
	// else
...
HRESULT __stdcall CDataObject::QueryGetData (FORMATETC *pFormatEtc)
{
	...
	return canRenderData(pFormatEtc) ? S_OK : DV_E_FORMATETC;
}
...
HRESULT __stdcall CDataObject::EnumFormatEtc (DWORD dwDirection, IEnumFORMATETC **ppEnumFormatEtc)
{
	if(dwDirection == DATADIR_GET)
	{
		FORMATETC *formats;
		LONG numFormats;
		enumFormats(&formats, &numFormats);
		auto result = CreateEnumFormatEtc(numFormats, formats, ppEnumFormatEtc);
		delete formats; // was allocated with 'new' in enumFormats()
		return result;
